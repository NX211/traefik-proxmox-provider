...
When multiple nodes are configured, auto-generated router/service IDs are prefixed with the node `name` to prevent collisions (e.g. `cluster1-myvm-100` instead of `myvm-100`). User-specified names in traefik labels are **not** prefixed — you are responsible for using unique names across clusters.
